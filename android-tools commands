### android-tools commands
| **Command** | **Description** |
| --------------|-------------------|
| `adb tcpip 5555` | Forward the ADB server to your device. |
| `pacman -Qm` | Connect Over Wi-Fi. |
| `adb devices` | Verify connection. |
| `adb install your_app.apk` | Install app. |
| `adb disconnect` | Your device will no longer be connected over Wi-Fi. |
| `adb logcat` | If you want to see debug logs for your app. |
| `adb uninstall com.your.package.name` | Replace com.your.package.name with your app's package name. |
| `adb logcat` | If you want to see debug logs for your app. |
